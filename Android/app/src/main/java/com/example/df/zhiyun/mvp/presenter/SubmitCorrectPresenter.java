 ...
 */
@ActivityScope
public class SubmitCorrectPresenter extends BasePresenter<SubmitCorrectContract.Model, SubmitCorrectContract.View> {
    @Inject
    ...
    private int mCountType = Api.COUNT_TYPE_DATE;

    public int getCountTypeIndex(){
        return mCountType-1;
    }

    public void changeCountType(int index){
        mCountType = index+1;
    }

    @Inject
    public SubmitCorrectPresenter(SubmitCorrectContract.Model model, SubmitCorrectContract.View rootView) {
    ...
    public void requestData(boolean pullToRefresh) {
        mModel.homeworkState(mSubjectId,mClassId,mCountType,paginatorHelper.getPageIndex(pullToRefresh))
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    if (pullToRefresh)
                ...
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))//使用 Rxlifecycle,使 Disposable 和 Activity 一起销毁
                .subscribe(new ErrorHandleSubscriber<BaseResponse<List<HomeworkState>>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<List<HomeworkState>> response) {
                        if(response.isSuccess()){
                            List<HomeworkState> tempDatas = response.getData();
