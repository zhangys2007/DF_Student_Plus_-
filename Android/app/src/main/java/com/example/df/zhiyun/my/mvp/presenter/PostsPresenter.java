 ...
 */
@ActivityScope
public class PostsPresenter extends BasePresenter<PostsContract.Model, PostsContract.View> {
    @Inject
    ...
    @Inject
    public PostsPresenter(PostsContract.Model model, PostsContract.View rootView) {
...
//        requestData(true);
    ...
    public void requestData(boolean pullToRefresh) {
        mModel.getPosts(paginatorHelper.getPageIndex(pullToRefresh))
                ...
                .subscribe(new ErrorHandleSubscriber<BaseResponse<List<PushData>>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<List<PushData>> response) {
                        if(response.isSuccess()){
                            List<PushData> tempDatas = response.getData();
    ...
    public void clickItem(int position){
        Post item = (Post)mAdapter.getData().get(position);
        item.setStatus(1);
        mAdapter.notifyItemChanged(position);
        mRootView.showDialog(item.getNoticeContent());
    }
}
