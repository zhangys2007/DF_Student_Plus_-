 ...
 */
@FragmentScope
public class GradeScoreAvgPresenter extends BasePresenter<GradeScoreAvgContract.Model, GradeScoreAvgContract.View> {
    @Inject
...
//    private PaginatorHelper paginatorHelper = new PaginatorHelper();
    ...
    @Inject
    public GradeScoreAvgPresenter(GradeScoreAvgContract.Model model, GradeScoreAvgContract.View rootView) {
    ...
    public void requestData(boolean pullToRefresh) {
        mModel.findAverageContrast(mFz,mType,mGradeId,mSchoolId,mSubjId)
                ...
                .subscribe(new ErrorHandleSubscriber<BaseResponse<List<GradeAvg>>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<List<GradeAvg>> response) {
                        if(response.isSuccess()){
                            List<GradeAvg> tempDatas = response.getData();
