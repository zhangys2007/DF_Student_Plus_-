 ...
 */
@FragmentScope
public class GradeScoreSumaryPresenter extends BasePresenter<GradeScoreSumaryContract.Model, GradeScoreSumaryContract.View> {
    @Inject
...
//    private PaginatorHelper paginatorHelper = new PaginatorHelper();
    ...
    @Inject
    public GradeScoreSumaryPresenter(GradeScoreSumaryContract.Model model, GradeScoreSumaryContract.View rootView) {
    ...
    public void requestData(boolean pullToRefresh) {
        mModel.knowledgeGrasp(mFz,mType,mGradeId,mSchoolId,mSubjId)
                ...
                .subscribe(new ErrorHandleSubscriber<BaseResponse<List<ScoreCompearSumary>>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<List<ScoreCompearSumary>> response) {
                        if(response.isSuccess()){
                            List<ScoreCompearSumary> tempDatas = response.getData();
