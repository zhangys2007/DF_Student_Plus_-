 ...
 */
@ActivityScope
public class HistoryHomeworkAnalyPresenter extends BasePresenter<HistoryHomeworkAnalyContract.Model, HistoryHomeworkAnalyContract.View> {
    @Inject
    ...
    @Inject
    public HistoryHomeworkAnalyPresenter(HistoryHomeworkAnalyContract.Model model, HistoryHomeworkAnalyContract.View rootView) {
    ...
    private void processFilterData(List<FilterGrade> data){
        ...
        if(mFilterData.size() > 0){
            ...
            mGradeId = filterGrade.getGradeID();
    ...
    public void requestData(boolean pullToRefresh) {
        mModel.findHomeworkAnalysis(paginatorHelper.getPageIndex(pullToRefresh),mSubjectId,mClassId,mGradeId)
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    if (pullToRefresh)
                ...
                .subscribe(new ErrorHandleSubscriber<BaseResponse<List<JsonObject>>>(mErrorHandler) {
                    ...
                    public void onNext(BaseResponse<List<JsonObject>> response) {
                        if(response.isSuccess()){
                            paginatorHelper.onDataArrive(mAdapter,pullToRefresh,transform(response.getData()),null);
    ...
    private List<HistoryHomeworkAnalyMultipleItem> transform(List<JsonObject> list){
        List<HistoryHomeworkAnalyMultipleItem> result = new ArrayList<>();
        if(list != null && list.size() > 0){
            JsonObject item1 = list.get(0);
            JsonElement je = item1.get("teacherStatus");
            if(je != null){
                int teacherStatus = je.getAsInt();
                if(teacherStatus == HistoryHomeworkAnalyContract.View.TYPE_MANAGER){
                    for(JsonObject jsonObject: list){
                        HistoryHomeworkAnalyMultipleItem mi = new HistoryHomeworkAnalyMultipleItem(
                                HistoryHomeworkAnalyContract.View.TYPE_MANAGER,mGson.fromJson(jsonObject, HistoryHWAnalyManager.class));
                        result.add(mi);
                    }
                }else if(teacherStatus == HistoryHomeworkAnalyContract.View.TYPE_TEACHER){
                    for(JsonObject jsonObject: list){
                        HistoryHomeworkAnalyMultipleItem mi = new HistoryHomeworkAnalyMultipleItem(
                                HistoryHomeworkAnalyContract.View.TYPE_TEACHER,mGson.fromJson(jsonObject, HistoryHWAnalyTeacher.class));
                        result.add(mi);
                    }
                }
            }
