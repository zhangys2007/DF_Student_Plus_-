 ...
 */
@ActivityScope
public class HistoryKnowledgePointPresenter extends BasePresenter<HistoryKnowledgePointContract.Model, HistoryKnowledgePointContract.View> {
    @Inject
    ...
    @Inject
    public HistoryKnowledgePointPresenter(HistoryKnowledgePointContract.Model model, HistoryKnowledgePointContract.View rootView) {
    ...
    public void requestData(boolean pullToRefresh) {
        ...
        mModel.historyKnowledgePointsAnaly(paginatorHelper.getPageIndex(pullToRefresh),mSubjectId,mClassId)
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    if (pullToRefresh)
                ...
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))//使用 Rxlifecycle,使 Disposable 和 Activity 一起销毁
                .subscribe(new ErrorHandleSubscriber<BaseResponse<List<HistoryKnowledgePoint>>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<List<HistoryKnowledgePoint>> response) {
                        if(response.isSuccess()){
                            List<HistoryKnowledgePoint> tempDatas = response.getData();
