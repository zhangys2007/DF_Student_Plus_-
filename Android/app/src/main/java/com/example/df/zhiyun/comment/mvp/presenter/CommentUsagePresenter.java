 ...
 */
@ActivityScope
public class CommentUsagePresenter extends BasePresenter<CommentUsageContract.Model, CommentUsageContract.View> {
    @Inject
    ...
    @Inject
    public CommentUsagePresenter(CommentUsageContract.Model model, CommentUsageContract.View rootView) {
    ...
    public void requestData(boolean pullToRefresh) {
        ...
        mModel.goTCommentonUsageHistoryTable(mUserName,params,mCountType,paginatorHelper.getPageIndex(pullToRefresh))
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    if (pullToRefresh)
