 ...
 */
@FragmentScope
public class KnowledgeScoreAnalyPresenter extends BasePresenter<KnowledgeScoreAnalyContract.Model, KnowledgeScoreAnalyContract.View> {
    @Inject
...
//    String mHomeworkId;
    ...
    @Inject
    public KnowledgeScoreAnalyPresenter(KnowledgeScoreAnalyContract.Model model, KnowledgeScoreAnalyContract.View rootView) {
    ...
    private void processData(CompearItem data){
        ...
        //添加标题
        list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_TITLE_2,"知识点"));
        list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_TITLE_1,"知识点得分"));
        list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_TITLE_1,"题号"));
        list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_TITLE_1,"年级得分率"));
        ...
        collumCount = 2+1+1+1+ (data.getClassList()==null?0:data.getClassList().size())*2;

        //添加数据项
        int i=0;
        for(CompearQuestion cq:data.getGradeQuestionList()){
            list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_VALUE_2,cq.getKnowledge()));
            list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_VALUE_1,
                    String.format("%.1f",cq.getScore())));
            list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_VALUE_1,
                    Integer.toString(i+1)));
            list.add(new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_VALUE_1,
                    String.format("%.1f%%",cq.getScoreRate())));
            for(CompearCla cla: data.getClassList()){
                QuestionCompearMultipleItem questionCompearMultipleItem =
                        new QuestionCompearMultipleItem(QuestionCompearMultipleItem.TYPE_VALUE_2, "--");
                ...
                for(CompearScore cScore: data.getClassScoreList()){
                    if(TextUtils.equals(cScore.getHomeworkId(),cla.getHomeworkId()) &&
                            TextUtils.equals(cScore.getKnowledgeId(), cq.getKnowledgeId())){
